...
## 2.81.8 (March 26, 2025) [stable-rc]
...
* CHANGE: When you open an org in a browser with the `org open` command, Salesforce CLI now generates a single-use frontdoor URL. This URL can be used one time only; subsequent use won't allow you to log into the org. This change makes the access to your org more secure.  Code Builder users: when you run `org open` in a Code Builder terminal, the URL to open an org in a browser is outputted rather than a browser automatically starting; this is normal expected behavior. The change is that the outputted URL is now single-use only. 
