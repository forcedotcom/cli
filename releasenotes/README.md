...
* FIX: If you run a command that uses the SOAP API, such as `project deploy start`, and you authorized the org using a JWT-based access token, the command now correctly fails and outputs an error message saying that the SOAP API doesn't support JWT-based access tokens.  (GitHub issue [#3297](https://github.com/forcedotcom/cli/issues/3297), jsforce PR [#1698](https://github.com/jsforce/jsforce/pull/1698))
